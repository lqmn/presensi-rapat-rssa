...
function showPeserta(id_rapat, dt) {
	var html = '<div class=""><h4>Peserta</h4><table id="tabelpeserta"class="table borderless table-nonfluid"><tr><th>NIP</th><th>Nama</th><th>INSTITUSI</th></tr>';
	...
	$.ajax({
		...
		success:function(data){
			...
			$.each( databaru, function( key, val ) {
				html= html+'<tr><td>'+val['NIP']+'</td><td>'+val['NAMA']+'</td><td>'+val['INSTITUSI']+'</td></tr>';
			});
			html = html+'</table></div>';
			dt.child(html).show();
		}
...
$(document).ready(function(){
	$('#jadwal-detail-nav').addClass("active");
